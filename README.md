...
This is a full **NFL season prediction** and sim engine built **end to end**. It is trained on XGBoost model that predicts every 2026 game and a Monte Carlo engine that runs the season 500 times to compute realistic playoff and Super Bowl odds. 

![unaviable](screenshots/1MonteCarloSIm.png)

Includes a interactive Streamlit app built-in to allow you to run your own sims with downloadable .csv's.
...
2. Runs the full season 500 times (Monte Carlo simulation) to produce stable and realtics playoff and Super Bowlprobabilities for all 32 teams
...
- **Monte Carlo**: 500 full season sims each with fresh randomness to make accurate playoff and Super Bowl odds
...
## Home - Standings & Odds
League wide player and Super Bowl odds based on teh 500 Monte Carlo sims
